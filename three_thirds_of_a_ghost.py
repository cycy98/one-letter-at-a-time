...
class ThreeThirdsOfAGhost:
    def __init__(self, players: list[discord.Member | discord.User], max_mistakes: int = 5) -> None:
        self.players = players
        self.current_prompt = ""
        self.turn = 0
        self.mistakes = dict.fromkeys(players, 0)
        self.ghostthirds = dict.fromkeys(players, 0)
    ...
    def add_ghostthird(self, player: discord.Member | discord.User) -> bool:
        self.ghostthirds[player] += 1
        return self.ghostthirds[player] >= 3
    ...
    async def handle_message(self, message: discord.Message) -> bool:
        ...
        if self.is_message_valid(content, author):
            if self.is_input_valid(content):
                ...
                if iswords(self.current_prompt):
                    await message.add_reaction("👻")
                    eliminated = self.add_ghostthird(author)
                    if eliminated:
                        await message.channel.send(f"{author.mention} has been turned into a ghost!")
                        self.eliminate(author)
                        if len(self.players) == 1:
                            await message.channel.send(f"{self.current_player().mention} wins!")
                            return True
                    await message.channel.send(f"{self.current_prompt} is a word, resetting to {self.reset_prompt()}")
                elif isfinal(self.current_prompt):
                    await message.add_reaction("👻")
                    eliminated = self.add_ghostthird(author)
                    if eliminated:
                        await message.channel.send(f"{author.mention} has been turned into a ghost!")
                        ...
                        if len(self.players) == 1:
                            await message.channel.send(f"{self.current_player().mention} wins!")
                            return True
                    await message.channel.send(f"{self.current_prompt} doesn't start a word, resetting to {self.reset_prompt()}")
                else:
                    await message.add_reaction("\u2705")
                self.next_turn()
            else:
