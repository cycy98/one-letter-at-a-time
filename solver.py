...
def solver_ghost(prompt: str) -> list[str]:
    prompt = prompt.casefold()
    return [prefix for prefix in PREFIXES if prefix.startswith(prompt) and
            len(prefix) == len(prompt)+1 and
            (len(prefix) <= 3 or prefix not in WORDLIST)]

def solver_olaat(prompt: str) -> list[str]:
    prompt = prompt.casefold()
    return [prefix for prefix in PREFIXES if prefix.startswith(prompt) and
            len(prefix) == len(prompt)+1]
